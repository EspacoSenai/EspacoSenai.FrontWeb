        ...
        <select
          ...
          onChange={(e) => {
            ...
            const selectedItem = itens.find(
              (item) => normalizarItem(item, 0).id === selectedId
            );
            if (selectedItem && onChange) {
              const info = normalizarItem(selectedItem, 0);
              onChange(info.id, info.raw);
            }
          }}
          className="w-full max-w-md px-4 py-3 rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-[#1a1a1a] text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#C61919] transition-all duration-150"
        >
          ...
          {itens.map((item, index) => {
            ...
            return (
              ...
              >
                {info.label} {disabled ? "(Indisponível)" : ""}
              </option>
            );
          })}
        </select>
