...
export default function CriarSala() {
  ...
  function handleTimeBlur(text, setText, setTime, fallback) {
    ...
    // Se digitou 1-2 dígitos, considera como horas
    if (digits.length <= 2) {
    ...
    // Se digitou 3-4 dígitos, divide em HH:MM
    const hh = clamp(Number(digits.slice(0, 2)), 0, 23);
    const mm = clamp(Number(digits.slice(2, 4)), 0, 59);
    setTime({ h: hh, m: mm });
        ...
        <form onSubmit={criar} className="space-y-8">
              ...
              <div className="relative bg-gray-200 rounded-md border border-gray-200 p-3 flex items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    onClick={() => {
                      const next = incTime(inicio, "m", +15);
                      setInicio(next);
          ...
          <div className="mt-12 ml-[10px]">
            <div className="flex items-center flex-wrap gap-3 md:gap-4"> 
              ...
              <input
                ...
                min={1}
                max={40}
                className="w-[80px] md:-ml-[900px] h-10 rounded-md bg-gray-200 border border-gray-200 px-3 text-[15px] text-black text-center shadow-sm outline-none focus:ring-2 focus:ring-[#B10404]"
                value={capacidade}
                placeholder="1"
          ...
          <div className="pt-4 flex justify-end">
            <button
              type="submit"
              disabled={!formValido}
              className={`min-w-[140px] rounded-md bg-[#B10404] text-white py-1.5 px-6 text-base shadow-sm hover:opacity-95 disabled:opacity-60 transition-opacity`}
            >
              Criar
            </button>
          </div>
