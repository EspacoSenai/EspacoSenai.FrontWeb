...
export default function EditarSala() {
  ...
  // Estados para seleção individual de PC
  const [pcSelecionado, setPcSelecionado] = useState(null);
  const [catalogosPorPC, setCatalogosPorPC] = useState({});
  ...
  // BUSCAR CATÁLOGOS DO AMBIENTE OU PC SELECIONADO
  useEffect(() => {
    // Se for grupo de computadores e tiver PC selecionado, buscar catálogo daquele PC
    const idParaBuscar = isGrupoComputadores && pcSelecionado 
      ? pcSelecionado 
      : ambienteIdNumber;
    ...
    (async () => {
      try {
        ...
        console.log(`[EditarSala] Carregando catálogos do ambiente/PC ID ${idParaBuscar}...`);
        const catalogosData = await buscarCatalogosPorAmbiente(idParaBuscar);
  ...
  }, [ambienteIdNumber, isGrupoComputadores, pcSelecionado]);

  // Selecionar automaticamente o primeiro PC quando for grupo de computadores
  useEffect(() => {
    if (isGrupoComputadores && ambientesSincronizados.length > 0 && !pcSelecionado) {
      const primeiroPC = ambientesSincronizados[0];
      if (primeiroPC?.id) {
        setPcSelecionado(Number(primeiroPC.id));
      }
    }
  }, [isGrupoComputadores, ambientesSincronizados, pcSelecionado]);
  ...
  async function handleAdicionarHorario(e) {
    ...
    // Se for grupo de computadores, usar o PC selecionado
    const idParaSalvar = isGrupoComputadores && pcSelecionado 
      ? pcSelecionado 
      : ambienteIdNumber;

    const pcNome = isGrupoComputadores && pcSelecionado
      ? ambientesSincronizados.find(pc => Number(pc.id) === pcSelecionado)?.nome
      : null;

    const ambienteNome = pcNome || (ambienteDetalhes?.nome || ambienteSlug || nome || "").trim();
    ...
    const catalogoData = {
      idAmbiente: idParaSalvar,
      ambienteId: idParaSalvar,
      diaSemana,
    ...
    try {
      if (catalogoEditando) {
        ...
        setSucesso(`Horário atualizado com sucesso!${isGrupoComputadores && pcNome ? ` (${pcNome})` : ''}`);
      } else {
        // Adicionando novo catálogo
        await salvarCatalogos(idParaSalvar, catalogoData);
        setSucesso(`Horário adicionado com sucesso!${isGrupoComputadores && pcNome ? ` (${pcNome})` : ''}`);
      }

      // Recarregar catálogos do PC/ambiente correto
      const catalogosData = await buscarCatalogosPorAmbiente(idParaSalvar);
      setCatalogos(Array.isArray(catalogosData) ? catalogosData : []);
      ...
      const idParaRecarregar = isGrupoComputadores && pcSelecionado 
        ? pcSelecionado 
        : ambienteIdNumber;

      if (idParaRecarregar) {
        try {
          const catalogosData = await buscarCatalogosPorAmbiente(idParaRecarregar);
          setCatalogos(Array.isArray(catalogosData) ? catalogosData : []);
        } catch (reloadErr) {
        ...
        {isGrupoComputadores && ambientesSincronizados.length > 0 && (
          <div className="w-full max-w-3xl bg-white rounded-2xl shadow-lg px-8 py-7">
            ...
            <div className="flex flex-col gap-4">
              <div>
                <label className={TXT_LABEL}>Computador *</label>
                <select
                  value={pcSelecionado || ""}
                  onChange={(e) => {
                    const id = Number(e.target.value);
                    console.log(`[EditarSala] PC selecionado: ${id}`);
                    setPcSelecionado(id);
                  }}
                  className={CL_SELECT}
                  disabled={carregando}
                >
                  {ambientesSincronizados.map((pc) => {
                    const pcId = Number(pc?.id);
                    const pcNome = pc?.nome || `PC ${pcId}`;
                    return (
