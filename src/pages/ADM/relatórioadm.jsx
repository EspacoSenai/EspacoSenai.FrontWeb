    ...
    <div className="min-h-screen bg-white dark:bg-[#0B0B0B] transition-colors duration-300">
        ...
        <div className="w-full flex justify-between items-center px-4">
          ...
          <div className="flex items-center gap-14 mr-8"> 
            <button
              ...
              className="w-[53px] h-7 flex items-center justify-between rounded-full p-1 transition-all duration-300 relative -mr-1" // de -mr-2 para -mr-4
              style={{ backgroundColor: modoEscuro ? "#fff" : "#333" }}
            >
              <img
                src={Noite}
                alt="Modo escuro"
                className={`w-4 h-4 absolute left-7 transition-all duration-300 ${
                  modoEscuro ? "opacity-0" : "opacity-100"
                }`}
              />
              <img
                src={Sol}
                alt="Modo claro"
                className={`w-4 h-4 absolute right-7 transition-all duration-300 ${
                  modoEscuro ? "opacity-100" : "opacity-0"
                }`}
              />
              <div
                className={`w-4 h-4 rounded-full shadow-md transition-transform duration-300 ${
                  modoEscuro
                    ? "bg-black transform translate-x-7"
                    : "bg-white"
                }`}
              ></div>
            </button>
            <button
              ...
              type="button"
              className="p-0 bg-transparent hover:bg-transparent active:bg-transparent focus:outline-none focus:ring-0"
              style={{ background: "transparent" }}
            >
              <img
                src={modoEscuro ? BotaoSidebarDark : BotaoSidebar}
                alt="Menu"
                className="h-8 w-8 block select-none pointer-events-none"
              />
            </button>
          </div>
        </div>
      ...
      <main className="w-full pl-[20px] pr-6 sm:pr-12 lg:pr-24 xl:pr-[896px] py-8 text-[#000] dark:text-[#E5E5E5]">
        ...
        <section className="space-y-2 text-left text-[14px] md:text-[16px]">
          <p>
            <span className="font-semibold">Escola:</span> Escola SENAI Suíço-Brasileira
            "Paulo Ernesto Tolle"
          </p>
          <p>
            <span className="font-semibold">Período:</span> 20/08/2025 a 24/08/2025
          </p>
          <p>
            <span className="font-semibold">Coordenador Responsável:</span> Chile{" "}
          </p>
          <p>
            <span className="font-semibold">Data de Geração do Relatório:</span> 12/07/2025
          </p>
        </section>
