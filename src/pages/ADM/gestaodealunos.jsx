...
const GestaoDeAlunos = () => {
  ...
  const alunosData = useMemo(
    () => [
      { id: 1, nome: 'Gabriela Almeida', email: 'gabi@email.com', statusReserva: 'Pendente', statusAluno: 'Ativo', turma: 'Seduc1' },
      { id: 2, nome: 'João Pedro', email: 'joao@email.com', statusReserva: 'Confirmada', statusAluno: 'Ativo', turma: 'Seduc1' },
      { id: 3, nome: 'Marina Costa', email: 'marina@email.com', statusReserva: 'Pendente', statusAluno: 'Ativo', turma: 'Seduc2' },
      { id: 4, nome: 'Rafael Souza', email: 'rafa@email.com', statusReserva: 'Cancelada', statusAluno: 'Inativo', turma: 'Seduc2' },
      { id: 5, nome: 'Bianca Lima', email: 'bianca@email.com', statusReserva: 'Confirmada', statusAluno: 'Ativo', turma: 'FIC' },
      { id: 6, nome: 'Carlos Nunes', email: 'carlos@email.com', statusReserva: 'Pendente', statusAluno: 'Ativo', turma: 'Seduc1' },
      { id: 7, nome: 'Ana Silva', email: 'ana@email.com', statusReserva: 'Confirmada', statusAluno: 'Ativo', turma: 'FIC' },
      { id: 8, nome: 'Pedro Santos', email: 'pedro@email.com', statusReserva: 'Pendente', statusAluno: 'Ativo', turma: 'Seduc2' },
    ],
    []
  );
  ...
  const alunosFiltrados = useMemo(() => {
    ...
    return alunosData.filter(a => a.turma.toLowerCase().includes(q) || a.nome.toLowerCase().includes(q));
  }, [query, alunosData]);
  ...
  const statusBg = s =>
    ({ Pendente: 'bg-red-700', Confirmada: 'bg-green-600', Cancelada: 'bg-zinc-600' }[s] || 'bg-zinc-500');
    ...
    return (
      <div className="space-y-4">
        <h2 className="text-left text-lg font-normal text-gray-900 dark:text-gray-100">
          <span className="inline-block border-b-2 border-red-700 pb-[2px]">{turma}</span>
        </h2>

        <div ref={ref} className="flex gap-6 overflow-x-auto snap-x snap-mandatory pb-2 scroll-smooth no-scrollbar">
          {items.map(a => (
            <div
              key={a.id}
            ...
            >
              ...
              <div className="p-7 sm:p-8 relative z-10 flex flex-col items-center text-center gap-4 sm:gap-5">
                ...
                <div className="text-sm text-gray-900 dark:text-gray-100 font-medium">{a.nome}</div>

                <div className={`w-full ${statusBg(a.statusReserva)} text-white text-xs sm:text-sm py-2 rounded-md`}>
                ...
                <div className="w-full space-y-2 text-sm">
                  <div className="flex items-center justify-between text-gray-900 dark:text-white">
                    <span className="truncate">
                      Email: <span className="text-gray-500 dark:text-gray-400">{a.email}</span>
                    </span>
                    <span>
                      Status: <span className="text-gray-500 dark:text-gray-400">{a.statusAluno}</span>
                    </span>
                  </div>
                  <div className="text-gray-900 dark:text-white">
                    Turma: <span className="text-gray-500 dark:text-gray-400">{a.turma}</span>
                  </div>
                </div>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white dark:bg-black text-gray-900 dark:text-white transition-colors duration-300">
      {/* Header */}
      <header className="flex justify-between items-center px-8 py-5 bg-white dark:bg-black transition-colors duration-300">
        ...
        <nav className="flex items-center gap-6">
          ...
          >
            ...
            />
            <div
              className={`w-4 h-4 rounded-full shadow-md transition-transform duration-300 ${isDarkMode ? 'bg-black transform translate-x-7' : 'bg-white'}`}
            />
          </button>

          {/* Menu Button */}
          <button
            className="p-0 m-0 bg-transparent border-none shadow-none outline-none hover:bg-transparent active:bg-transparent focus:bg-transparent"
            aria-label="Menu"
            style={{ boxShadow: 'none', background: 'none' }}
          >
            <img
              src={isDarkMode ? BotaoSidebarDark : BotaoSidebar}
              alt="Abrir menu"
              className="h-7 w-7"
              draggable={false}
            />
      ...
      <div className="relative z-10 w-full h-px bg-gray-300 dark:bg-white -mt-2" />
      ...
      <main className="flex-1 flex flex-col">
        ...
        <section className="flex-1 bg-red-700 text-white px-4 sm:px-8 py-12 sm:py-16 lg:py-20 text-center flex items-center justify-center mt-10">
          <div className="max-w-2xl mx-auto">
            <h1 className="text-2xl sm:text-3xl font-semibold mb-6 tracking-tight text-center">Gestão de alunos</h1>
            <p className="text-base sm:text-lg font-normal leading-relaxed mb-3 text-center md:transform md:-translate-y-1 md:-translate-x-9 whitespace-normal md:whitespace-nowrap">
              Aqui você pode acompanhar e organizar as informações dos alunos com facilidade!
            </p>
