...
// Removido import duplicado e cancelarReserva; usamos deletarReserva
...
const Kebab = ({ onClick, ariaControls, ariaExpanded }) => (
  <button
    ...
    onClick={onClick}
    className="h-12 w-12 grid place-items-center rounded-xl bg-transparent text-neutral-700 dark:text-white hover:bg-black/5 transition outline-none focus:outline-none"
    style={{
  ...
  const handleCancelar = async () => {
    ...
    try {
      ...
      if (errorMessage.includes("já foi cancelada") || errorMessage.includes("Esta reserva já foi cancelada") || errorMessage.includes("No Content")) {
        // Mesmo assim chama onCancelado para atualizar a lista
        if (onCancelado) onCancelado();
      }
      // Para outros erros, não faz nada (não mostra alert)
    } finally {
      setLoading(false);
    }
  };
