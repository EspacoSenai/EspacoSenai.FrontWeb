...
/* ===== IDs fixos ===== */
// ID do AMBIENTE do PS5 (ajuste conforme estiver no banco)
...
export default function AgendamentoPS5() {
  ...
  // catálogo do PS5 vindo do back (lista de objetos com id, diaSemana, horaInicio, horaFim)
    ...
    async function carregarCatalogoPS5() {
      try {
        ...
        const mapeado = data.map((item) => {
          ...
          // PS5: ambienteId = PS5_AMBIENTE_ID OU nome contendo “ps5” ou “playstation”
          const isPS5 =
        ...
        console.log("[AgendamentoPS5] catálogo completo:", mapeado);

        const apenasPS5 = mapeado.filter((it) => it.isPS5 && it.isDisponivel);

        console.log("[AgendamentoPS5] apenas PS5:", apenasPS5);
  ...
  async function confirmar() {
    ...
    // >>>>>> escolhe o catálogo corretinho pelo dia + horário <<<<<<
    const catalogoSelecionado = catalogoPS5.find(
      (c) =>
    ...
    if (!catalogoSelecionado?.id) {
      console.error(
        "[AgendamentoPS5] Não encontrou catálogo para",
        diaSemanaBackSelecionado,
        horaInicio,
        horaTermino
      );
    ...
    const catalogoId = catalogoSelecionado.id || PS5_CATALOGO_FALLBACK_ID;
