...
export default function Avatares() {
  ...
  function handleConfirm() {
    ...
    if (found) {
      // salva escolha pro Header usar
      localStorage.setItem("avatar_id", found.id);
      localStorage.setItem("avatar_url", found.src);
      // dispara evento de storage em SPA (atualiza header já)
      window.dispatchEvent(new StorageEvent("storage", { key: "avatar_url", newValue: found.src }));
    }
    navigate(-1);
  }
  ...
  return (
    <div className="min-h-screen bg-white font-sans">
      ...
      <div className="relative flex items-center justify-center px-3 py-3 border-b border-gray-200">
        <button
          onClick={() => navigate(-1)}
          className="absolute left-1 top-1 bg-white rounded-md border border-gray-300 shadow-sm p-1
      ...
      <main className="max-w-3xl mx-auto px-6 py-8">
        <div className="grid grid-cols-3 gap-8">
          {avatars.map((a) => (
            <button
              ...
              className={`flex items-center justify-center h-40 w-40 bg-[#D1D5DB] rounded-lg shadow-sm relative ${
                selected === a.id ? "ring-4 ring-[#b91c1c] scale-105" : ""
              }`}
            >
        ...
        <div className="mt-12 flex justify-end">
          <button
            ...
            disabled={!selected}
            className={`bg-[#b91c1c] text-white px-6 py-1.5 rounded-md ${
